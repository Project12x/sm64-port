...
- Isolated donor worktree `d7b04d61` bound only canonical ROM (SHA-256
  ...
  environment. Its historical tool environment and top-level asset route reached
  `extract_assets.py`, then stopped on missing Windows `./tools/mio0`; no CUE,
  ELF completion, Ymir launch, video, or live attempt occurred.
...
- Task 2 is blocked. Tasks 3–6 remain closed. Do not repair the actor stack,
  Make graph, or historical host tool path without a new explicit product-boundary
  decision; the archived A9A CUE remains the only visual/control fallback.
