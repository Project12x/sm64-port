    ...
    uint16_t count)
{
    ...
    for (index = 0U; index < count; index++) {
        ...
        const sm64_saturn_actor_instance_descriptor_t *const supplied =
            &descriptors[index];
        if (supplied->snapshot_index >= count ||
    ...
    uint16_t batch_capacity)
{
    ...
    if (descriptor_count != 0U && !sm64_saturn_actor_instance_queue_publish(
            queue, generation, manifest_capacity, output_record_capacity,
            descriptors, descriptor_count)) {
        quarantine_after_acquire(handoff);
        return false;
    }
    handoff->queue_published = descriptor_count != 0U;
...
bool sm64_saturn_actor_runtime_handoff_finalize(
    sm64_saturn_actor_runtime_handoff_t *handoff)
{
    if (handoff == NULL || handoff->state != SM64_SATURN_ACTOR_HANDOFF_QUEUED)
        return false;
    if (handoff->queue_published &&
    ...
    if (handoff->queue_published && !sm64_saturn_actor_batches_build(
    ...
    sm64_saturn_actor_runtime_handoff_t *handoff)
{
    if (handoff == NULL || handoff->state != SM64_SATURN_ACTOR_HANDOFF_CONSUMED ||
        handoff->output_consumer_acknowledged == 0U)
        return false;
    if (handoff->queue_published && !sm64_saturn_actor_instance_queue_reset_retired(
            handoff->queue, handoff->generation))
        return false;
