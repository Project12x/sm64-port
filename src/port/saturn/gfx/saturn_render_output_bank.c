...
static inline sm64_saturn_render_job_queue_t *output_queue_cache_through(
    sm64_saturn_render_job_queue_t *queue)
{
    return (sm64_saturn_render_job_queue_t *)
        sm64_saturn_dual_frame_cache_through(queue);
}
...
{
    bank = output_bank_cache_through(bank);
    queue = output_queue_cache_through(queue);
    if (bank == NULL || queue == NULL || job_index >= queue->count ||
        job_index >= SM64_SATURN_RENDER_JOB_QUEUE_CAPACITY)
        return false;
    sm64_saturn_render_job_release_t *const queue_release =
        &queue->release[job_index];
    if (!output_claim_try(&queue_release->claim)) return false;
    output_bank_fence();
    const sm64_saturn_render_job_t *const job = &queue->jobs[job_index];
    const sm64_saturn_render_job_state_t claimed_state =
        (sm64_saturn_render_job_state_t)queue_release->state;
    const sm64_saturn_render_output_bank_kind_t kind =
        sm64_saturn_render_output_bank_kind_for_job(job);
    if (queue->generation != job->snapshot_generation ||
        queue_release->generation != job->snapshot_generation ||
        job->snapshot_generation == 0U || !output_claimed_state_valid(claimed_state) ||
        !output_kind_valid(kind) || bank->kind != (uint32_t)kind) {
        output_claim_release(&queue_release->claim);
        return false;
    }

    sm64_saturn_render_output_release_t *const release =
        &bank->release[job_index];
    if (!output_claim_try(&release->claim)) {
        output_claim_release(&queue_release->claim);
        return false;
    }
    output_bank_fence();
    ...
    uint8_t *owner_lane)
{
    ...
    queue = output_queue_cache_through((sm64_saturn_render_job_queue_t *)queue);
    if (bank == NULL || queue == NULL || owner_lane == NULL ||
    ...
    const sm64_saturn_render_output_bank_kind_t kind =
        sm64_saturn_render_output_bank_kind_for_job(job);
    if (job->snapshot_generation == 0U ||
        queue->generation != job->snapshot_generation ||
        queue->release[job_index].generation != job->snapshot_generation ||
        !output_kind_valid(kind) || bank->kind != (uint32_t)kind)
        return false;
    ...
    uint8_t reader_lane)
{
    ...
    if (reader_lane > SM64_SATURN_RENDER_OUTPUT_LANE_SLAVE ||
        !sm64_saturn_render_output_bank_owner_lane(bank, queue, job_index,
                                                   &owner_lane))
        return false;
