 ...
 * ZT_RENDERING.c:439-480 and ZT_LOADING.c:148-172. It deliberately does not
...
/* Tracks the source game scene boundary independently of renderer frame
 * cadence. A NULL current area is an exit, so a subsequent equal level/area
 * pair is still a re-entry and cannot inherit stale tier state. */
typedef struct saturn_lod_scene {
    int16_t level;
    int16_t area;
    uint8_t active;
} saturn_lod_scene_t;
