...
static sm64_saturn_desired_voice_t desired(uint32_t generation, uint32_t note_id)
{
    ...
    assert(desired.envelope_word == 0x075eU);
    assert(desired.release_word == 0x017cU);
...
static void test_reassignment_and_stale_generation_fail_closed(void)
{
    sm64_saturn_slot_shadow_t shadow;
    sm64_saturn_slot_command_t commands[SM64_SATURN_SLOT_MAX_COMMANDS];
...
int main(void)
{
    ...
    test_reassignment_and_stale_generation_fail_closed();
