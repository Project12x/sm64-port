...
static void test_fused_terrain_publication_preserves_private_dynamic_commands_and_post_light_shades(void)
{
    ...
    const int16_t primitive_3_triangle[4][2] = {
        {-7, 8}, {9, 8}, {1, 19}, {1, 19}};
    const uint16_t post_light_shades[4] = {
        0x801FU, 0x83E0U, 0xFC00U, 0x83FFU};
    ...
    assert(sm64_saturn_terrain_template_patch_resolved_record(
        ...
        0U, false, false, 0U));
    /* The worker result owns only dynamic payload: optional post-light
     * shades at bytes 0..7 and coordinates at bytes 12..27. Immutable
     * material is copied from the resolved template by the master after the
     * cross-CPU join. */
    ...
    assert(sm64_saturn_terrain_result_publish(
        ...
        NULL, primitive_2));
    assert(sm64_saturn_terrain_result_publish_with_shades(
        &spans.slave, 3U, 18U, 500U, 3U,
        SM64_SATURN_TERRAIN_CLIP_CROSSES |
            SM64_SATURN_TERRAIN_RESULT_POST_LIGHT_SHADES,
        &resolved, post_light_shades, primitive_3_triangle));
    assert(sm64_saturn_terrain_result_publish_with_shades(
        &spans.master, 1U, 17U, 500U, 4U,
        SM64_SATURN_TERRAIN_CLIP_FRONT,
        &resolved, post_light_shades, primitive_1));
    ...
    assert(sm64_saturn_terrain_result_has_post_light_shades(refs[1].record));
    ...
    assert(memcmp(worker_primitive_1 + 28U, zero_tail,
                  sizeof(zero_tail)) == 0);
    assert(memcmp(worker_primitive_3, post_light_shades,
                  sizeof(post_light_shades)) == 0);
    assert(memcmp(worker_primitive_3 + 8U, zero_tail,
                  sizeof(zero_tail)) == 0);
...
int main(void)
{
    ...
    test_fused_terrain_publication_preserves_private_dynamic_commands_and_post_light_shades();
