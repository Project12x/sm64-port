...
static void test_rejects_resealed_offset_past_package_without_reading_it(void)
{
    test_scene_fixture_t fixture; sm64_saturn_scene_package_view_t view;
    test_make_fixture(&fixture,0);
    test_write_u32(fixture.root+84U+8U,UINT32_MAX);
    test_seal_root(&fixture);
    assert(!sm64_saturn_scene_package_validate(fixture.root,fixture.root_size,&view));
    assert(view.root_bytes==NULL);
}

static void test_stable_id_generation_and_zero_payload_match_python_abi(void)
{
    test_scene_fixture_t fixture; sm64_saturn_scene_package_view_t view; uint8_t digest[32]; uint32_t offset;
    test_make_fixture(&fixture,0); offset=test_read_u32(fixture.root+84U+4U*64U+8U)+4U;
    fixture.root[offset+4U]=0xFFU; test_reseal_package(&fixture);
    assert(!sm64_saturn_scene_package_validate(fixture.root,fixture.root_size,&view));
    test_make_fixture(&fixture,0); offset=test_read_u32(fixture.root+84U+4U*64U+8U)+4U;
    test_write_u32(fixture.root+offset+84U,0U); test_reseal_package(&fixture);
    assert(sm64_saturn_scene_package_validate(fixture.root,fixture.root_size,&view));
    test_write_u32(fixture.root+offset+36U,0U);
    assert(sm64_saturn_scene_package_sha256(NULL,0U,digest)); memcpy(fixture.root+offset+52U,digest,32U);
    test_reseal_package(&fixture);
    assert(sm64_saturn_scene_package_validate(fixture.root,fixture.root_size,&view));
}
...
static void test_provisional_is_explicit_and_zero_section_is_valid(void)
{
    ...
    assert(!sm64_saturn_scene_package_validate_target(fixture.root,fixture.root_size,&view));
    assert(view.root_bytes==NULL && view.section_count==0U);
...
static void test_sha256_known_vector(void)
{
    ...
    uint8_t actual[32]; assert(sm64_saturn_scene_package_sha256("abc",3U,actual)); assert(memcmp(actual,expected,32U)==0);
    assert(!sm64_saturn_scene_package_sha256(NULL,1U,actual));
}
...
int main(void) { test_sha256_known_vector(); test_validates_big_endian_root_and_dependencies();
    ...
    test_rejects_resealed_offset_past_package_without_reading_it();
    test_stable_id_generation_and_zero_payload_match_python_abi();
