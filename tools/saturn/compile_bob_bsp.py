...
def compile_bsp(scene: dict[str, object], candidate_limit: int = 32,
    ...
    leaf_spans = _leaf_spans(ranges, refs)
...
def _leaf_spans(ranges: list[tuple[int, int]], refs: list[int]) -> dict[str, object]:
    ...
    return {
        ...
        "leaf_first_ref": first_ref,
        "leaf_ref_count": ref_count,
...
def header_text(scene: dict[str, object], candidate_limit: int = 32,
    ...
    lines += [
        ...
        "#define SM64_SATURN_BOB_LEAF_SPAN_COUNT SM64_SATURN_BOB_BSP_NODE_COUNT",
        f"#define SM64_SATURN_BOB_PRIMITIVE_REF_COUNT {leaf_spans['primitive_ref_count']}U",
        "static const uint16_t sm64_saturn_bob_leaf_first_ref[SM64_SATURN_BOB_LEAF_SPAN_COUNT] = {",
    ]
    lines.extend("    %dU," % value for value in leaf_spans["leaf_first_ref"])
    lines += [
        "};",
        "static const uint16_t sm64_saturn_bob_leaf_ref_count[SM64_SATURN_BOB_LEAF_SPAN_COUNT] = {",
    ]
    lines.extend("    %dU," % value for value in leaf_spans["leaf_ref_count"])
    lines += [
        "};",
    ...
    lines.extend(
        "    " + ", ".join(str(value) + "U" for value in
                            leaf_spans["primitive_refs"][offset:offset + 16]) + ","
        for offset in range(0, leaf_spans["primitive_ref_count"], 16))
    lines += [
        "};",
