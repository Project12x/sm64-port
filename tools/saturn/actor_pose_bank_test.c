...
int main(int argc, char **argv)
{
    ...
    if (!sm64_saturn_actor_bank_workspace_requirements(
            1U, 1U, &lane_bytes, &usable_bytes, &reserved_bytes) ||
        lane_bytes != 76U || usable_bytes != 152U || reserved_bytes != 155U ||
        !sm64_saturn_actor_bank_workspace_requirements(
            2U, 1U, &lane_bytes, &usable_bytes, &reserved_bytes) ||
        lane_bytes != 88U || usable_bytes != 176U || reserved_bytes != 179U ||
        sm64_saturn_actor_bank_workspace_requirements(
            0U, 1U, &lane_bytes, &usable_bytes, &reserved_bytes)) {
    ...
    if (!sm64_saturn_actor_bank_validate(bytes, size, &view) ||
        ...
        view.bank.meshlet_count != 31U || view.max_scratch != 11043U) {
