...
| 2026-08-04 | A4 actor ordering is bounded by generated material/opacity meshlets, not a global primitive comparison sort. | Original project code generates at most 32 source-ordered primitives per meshlet with tight actor-local bounds and compact near/mid/far primitive/position lists. The serial master rejects behind meshlets before actor transform dispatch, transforms each admitted position once, emits opaque meshlets in stable source order, and uses 64 stable far-to-near bins only for textured/translucent references. Gouraud, texture slots, terrain-relative insertion, VDP1, and presentation remain master-owned. This is host-source evidence only; independent review and target visual/counter evidence remain open. |
