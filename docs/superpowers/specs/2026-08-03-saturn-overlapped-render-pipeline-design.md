...
| 2026-08-04 | Queue claimant ownership must be bridged to exact descriptor-indexed result reads before any fixed terrain/Mario worker is replaced. | A5.5's bridge maps descriptor index + kind + actual queue claim to cached/P2 selection and rejects consumers before that exact job is `DONE`; it reserves one queue lifecycle attachment but compiles no CPU-DUAL registration/notify beside the legacy worker. The current renderer still uses fixed splits, so no live binding or target claim follows from this source-only bridge. |
