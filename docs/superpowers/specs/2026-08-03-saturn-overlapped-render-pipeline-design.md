...
| 2026-08-03 | A1 suppresses only source scene construction (`geo_process_root`, viewport/HUD/text/scissor emission) on Saturn IR ticks. Cutscene, credits, menus, warp-transition progression, and exit cleanup remain source-owned. | Implemented in `4a8fe1ce`; independent review and controller-owned CUE/Ymir gate pending. |
| 2026-08-03 | The shared `render_game()` guard must use definitions available in ordinary non-Saturn translation units as well as Saturn builds. | Corrected with project-native `s32`/`FALSE` in `9904097e`; the N64-shaped `-nostdinc` syntax gate passes through the MSYS wrapper. |
