...
| 2026-08-03 | The shared `render_game()` guard must use definitions available in ordinary non-Saturn translation units as well as Saturn builds. | Corrected with unconditional `<stdbool.h>` in `658d5ad9`; non-Saturn syntax gate passes through the MSYS wrapper. |
