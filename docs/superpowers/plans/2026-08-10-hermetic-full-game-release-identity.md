...
### Repair round 2 — race-resistant extracted-asset cleanup (2026-08-11)

The final rereview found an ancestor-swap window between
`_clean_asset_path()` and path-based `unlink()`/`rmdir()`. Reference-first
inspection covered Task 7 repair rounds 2/3, its full
`DirectoryNamespaceGuard`, and the `path_identity` exact-opened-object
publication pattern. Reuse mode is same-repository close-port: extracted-asset
cleanup now holds Task 7's full-ancestor guard across validation and mutation,
uses directory-relative removal under the held POSIX descriptor, and deletes
the exact identity/type/reparse-checked opened handle on Windows. Missing
directory-relative capability fails closed. Batch cleanup defers pruning until
file removals finish, then removes only proven empty descendants below the
root through the same guarded primitive.

TDD RED ran seven focused tests with two expected failures: a post-validation
ancestor replacement deleted the outside file, and an unavailable namespace
capability still deleted the generated target. Authoritative Windows GREEN is
7/7 with one symlink-capability skip; the deterministic junction race preserves
both outside/generated bytes and the output root. Shared Task 7 regressions are
release-manifest 28/28 and staging 20/20. Behavior commit is `df7894ac`.

Both clean owned candidates then rebuilt at `df7894ac` with the exact profile,
flag tuple, `verify-sourceboot`, and `-j1`; they exited 0 in 924.0/937.2
seconds, independently verify at manifest `9110b40d...b99`, identity
`id-a40f992c085da2f0`, and compare identical with zero differing fields.
Unsealed measurement passed at total 700 with both forbidden callers absent.
The 606-byte v4 contract is `14db6bfb...b8b` (pin commit `537dd721`), and
exact v4 passed at result `2e37d74b...c286`. Historical v2/v3 bytes remain
unchanged.

Capacity and package gates pass: `___end=0x060fca38` leaves 13,768 physical
and 6,856 usable HWRAM bytes; the 3,565,696-byte cart payload leaves 628,608
bytes; all three `SOURCE.DAT` copies match; package root `85a5a190...c266`
contains exactly ten BOB classes. Capacity 208 exceeds the prior artifact-bound
idle-boot peak 138 by 70, but 138 remains a floor without pickup/hold or
action-particle coverage. The canonical five-file/17,982,718-byte stage
verifies manifest `9110b40d...b99`; a repeated stage refused overwrite and
left its inventory unchanged.

Fresh post-pin verification is extractor 7/7 (one capability skip), checkout
1/1, attestation 17/17, hermetic Make 19/19, bootstrap 15/15, closure 27/27
(one capability skip), release manifest 28/28, staging 20/20, sealer 12/12,
and native math 243/244 with only the approved pre-existing null-camera proof
failure. Task 9 is `source-complete`; both same-reviewer rereviews remain open.
Task 10 is the next acceptance lane and remains paused. No Ymir, smoke, visual,
desktop, or manual gate ran.
