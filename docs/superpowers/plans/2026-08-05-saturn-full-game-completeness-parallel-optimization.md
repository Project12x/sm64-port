...
- [ ] RED: cover acquire/validate/publish/finalize/consume/retire ordering, exact descriptor identity preservation, stale/wrong bank rejection, pre-acquire immutability, post-acquire quarantine cleanup, terminality before batching, explicit output-consumer acknowledgement before reset/retire, double-operation/generation-wrap rejection, and exact capacity/overflow boundaries.
- [ ] Implement the `EMPTY -> ACQUIRED -> QUEUED -> TERMINAL -> BATCHED -> CONSUMED -> RETIRED` lifecycle using existing Task 14 bank, Task 16 queue, and batch types; no parallel ABI or synthetic geometry fields.
- [ ] Run serialized GREEN for `verify-actor-runtime-handoff verify-actor-instance-snapshot verify-actor-instance-queue verify-actor-batches verify-render-snapshot-bank` plus `test_actor_runtime_neutrality.py`; host-only evidence must leave P2/target/concurrent-claim/Ymir/manual/FPS gates unchecked.
- [ ] Commit and independently review the bounded handoff; keep Task 14 registry/typed-field defects, generic meshlet preparation, production drain, and target evidence explicitly open.
