...
- 2026-08-06 sequence-VM parity repair: Task 15 now persists small-layer velocity/duration state with source initialization, selects US versus EU/SH reserve opcodes explicitly, consumes reserve operands, matches EU/SH `bltz` polarity, and tracks d7/d6 active/finished plus test-channel-disabled state. The bounded fixture gate covers stack/loop/overflow/invalid-target/branch/layer regressions, but rereview still found large-layer note1 duration, d7 selected-bit preservation, and over-broad EU/SH format claims; a targeted repair is active and full package support remains blocked.
...
- [ ] Task 15 — active/source-incomplete — commits `df95a107`, `52d45d5e`, `211158ea`; bounded pointer-free VM scaffold and `verify-sequence-vm` are green, with Project12x ranges/provenance and source-parity repairs recorded. Independent rereview is SPEC/QUALITY FAIL, C1/I2; targeted large-note/d7/format-scope repairs are active. Full seq00/35-sequence/S64P/MC68000 image/SCSP/target/Ymir/manual audio gates remain blocked by Task 12 assets and later integration.
...
- [x] Commit bounded scaffold as `df95a107` with edge-semantics repairs `52d45d5e`, `211158ea`; independent review pending. No MC68000 image, SCSP, target, Ymir, manual, or full-content claim is made.
