...
- [ ] Task 19 — active/source-incomplete — close BOB articulated/enemy/boss capabilities; fix round 1 is in progress after independent SPEC/QUALITY FAIL (C0/I4/M1). The repair must separate spawn provenance from typed immutable parent identity, add the required pose/queue/lane/held/stale contract or explicitly defer it, pin all six exact query sets, reseal the stored-mask mutation, and run the brief’s complete serial regression wave. No production-capability or target/FPS claim is made until those gates and a scoped rereview pass.
- [ ] Task 20 — preflight complete; implementation not started — close BOB billboard/translucent/shadow/effect capabilities. The preflight report records five BILLBOARD, 30 ALPHA, 18 TRANSLUCENT, 34 SHADOW, zero PARTICLE, and zero DECAL records; nine effect-related records are still blocked by the two unsupported geo constructs. Source/target acceptance is explicitly split, and no effect capability or target/FPS claim is made.
