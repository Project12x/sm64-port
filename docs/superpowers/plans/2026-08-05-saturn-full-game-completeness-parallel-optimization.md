...
> **SUPERSEDED FOR EXECUTION — 2026-08-13.** This plan preserves research,
> measurements, commit provenance, and unfinished technical work, but it cannot
> authorize another task. Its regression-permitting policy and delayed
> second-level gate are explicitly replaced by
> [`../../saturn/PRODUCT_GOAL.md`](../../saturn/PRODUCT_GOAL.md) and
...
- 2026-08-06 Task 21 preflight correction: sourceboot audio plumbing is implementable but production-blocked. Feature-on sourceboot currently links no loader/service/transport/image, the weak event emitter drops events, the standalone soundtest owns the warned SNDOFF/SNDON helpers, the Task 17 engine is not linked into the production 68K loop, real seq00 and reproducible AUDIO.DAT are absent, and the provisional BOB S64P has empty audio dependencies. Task 21 must stop/fail closed without those inputs, use one project-owned CDFS/SNDOFF owner, stage only validated selected chunks within sound/cart bounds, and leave Task 23 as the first target/audible proof after Task 22 final-root reseal.
- 2026-08-13 BOB audible-convergence record: the normal sourceboot BOB path may activate only a closure-selected, generic SFX bundle. The actual BOB semantic subset is 54 mappings: 52 PCM-backed rows (63 descriptors because 11 effects layer two samples) and two source-authentic synthesized red-coin waveform rows which remain explicitly unsupported rather than being replaced by a proof tone. Its 44 distinct decoded PCM8 samples occupy 208,056 bytes in the fixed SCSP PCM bank `[0x00008000, 0x00080000)`, leaving 283,464 bytes; the 1,220-byte pointer-free descriptor table belongs in the existing `[0x00005000, 0x00008000)` driver-reserved span. The immutable bundle is generated from the selected scene closure and source-attested audio inputs, stored in source-cart read-only data, copied only at cold boot by the sole sourceboot sound-CPU owner, and then read by the MC68000 consumer through the existing pointer-free mailbox. The SH-2 semantic policy is the only producer; the MC68000 maps exact sound-bit identities to validated resident PCM and SCSP voices. Header, every row, source span, destination span, and total bank extent must validate before SNDOFF/SNDON; any failure leaves the driver muted and publishes no partial generation. The active sound generation is retained for the boot and must reject stale/unknown events. First evidence is an identity-bound normal BOB CUE with a semantic event reaching an actual selected PCM sample; sequence/music playback, replacement-scene residency, and full-catalog CD streaming remain explicitly open.
- 2026-08-13 SCSP cold-boot repair in progress: current semantic-audio BOB image `id-7394af…` black-screened while the same current source/package with audio compiled out (`id-bfe900…`) rendered under the same Ymir/BIOS replay. A first safe-order CUE (`id-c2e8129e…`) proved that the warm `SNDON` was not the immediate cause: its trace froze at bootstrap-retired after Ymir logged `SNDOFF`, before `SNDON`. The real fault was treating readback of the write-only SCSP 512-KiB mode latch as success. Sourceboot now keeps the CPU stopped for `SNDOFF → issued 512-KiB write → full 512-KiB clear → validated copies → final SNDON`, and the subsequent `READY`/heartbeat proves the live result. Physical ownership is unchanged: sourceboot writes `[0x25A00000, 0x25A80000)` only during cold boot; the immutable source-cart driver/bundle is the producer; the MC68000 mailbox/voice service is the first consumer; reset/failure leaves it muted with no partial generation. The next and only acceptance step is a fresh normal generic BOB CUE which both renders and produces an observed selected PCM SFX; no new audio architecture, package format, or synthetic proof path is authorized.
- 2026-08-06 custom-audio proposal audit: the four requested directions are compatible only as bounded source/host slices—(1) Yaul stays pinned/upstream behind one project-owned SMPC/sound-CPU wrapper, (2) the freestanding MC68000 driver remains a separate project target, (3) the pointer-free v2 mailbox gains completion/ack and package-identity status without a shared native voice table, and (4) deterministic asset conversion/residency/build identity continues fail-closed. The audit records Yaul `smc.h:93-102,199-215`, PoneSound `PROJ/main.c:507-577`/`jo_demo/pcmsys.c:250-272`, and the license-unclear Yaul example as pattern-only evidence. Missing seq00/full AUDIO.DAT, final BOB S64P, strong sourceboot bridge, and production MC68000 linkage still block semantic/audible claims.
- 2026-08-06 completion/ack ABI preparation: the next audio wave should add a pointer-free 32-entry, 16-byte completion ring in the reserved mailbox space, keyed by source ring plus full two-lap cursor, with explicit accepted/rejected/stale/fault/dropped/finished/prepared/committed statuses and 32-bit active/prepared package generations. The current 16-bit PLAY_REFRESH package-generation field versus Task12/17's 32-bit generation is a mandatory RED design decision; no silent truncation is allowed. The prep note is `.superpowers/sdd/2026-08-05-saturn-full-game-completeness-parallel-optimization/task-21-completion-ack-abi-prep.md`.
- 2026-08-06 actor feature-off boundary correction: `c763590b` routes the existing ACTOR_ADMIT/ACTOR_LOWER pair through explicit Mario compatibility wrappers when `SATURN_FEATURE_DYNAMIC_ACTOR_CLOSURE=0` and fails closed when it is enabled. Repair `8192572f` makes the source gate parse both preprocessor branches and mutation-proves feature-on delegation, feature-off failure, and lower-wrapper delegation. This is a source-contract preservation slice only; generic actor cutover, production drain, target/Ymir/manual, and FPS remain open.
- 2026-08-06 sourceboot launch correction: the pre-cart build-identity guard was reading `saturn_build_identity` from cart-resident `.rodata` and spinning at `main.c:1423` before `source_cart_load()`. The tuple now lives in HWRAM `.bootdata`, enforced by `tools/saturn/test_sourceboot_identity_residency.py`; fresh identity `e2-bob-identity-id-162e333360712826` reaches interactive BOB under the profile-managed 32-Mbit DRAM cart. This closes the obsolete black-screen launch blocker only. The post-BIOS trace decoder reports `0xfef1004a` because the active runtime overwrites the diagnostic marker; it is not target/FPS evidence. Textures, sound, final actor closure, and performance remain open.
- 2026-08-06 actor-runtime handoff acceptance: `963e10d4` plus fix `e82759ce` now owns the zero-count queue generation, enforces canonical snapshot/source ordinals, prevalidates worst-case batch capacity, retains terminal ownership for unexpected finalize failure, and records destructive queue reset before bank-retire retry. Independent rereview is SPEC/QUALITY PASS, C0/I0/M0. The handoff remains host/source-neutral; Task14 identity/typed-field, P2 target, production drain, Ymir/manual, and FPS gates remain open.
- 2026-08-06 Task14 P2 publication acceptance: `ce542f35` routes capture writes through the existing `actor_bank_uncached()` alias before the unchanged READY publication fence. The source mutation gate and exact acquired-payload fixture pass; independent rereview is SPEC/QUALITY PASS, C0/I0/M0. Host cache aliases collapse, so real SH-2 cache-race/P2 evidence, >64 source-pool identity, skip-zero generation, package reservation, production drain, Ymir/manual, and FPS remain open.
- 2026-08-06 Task 20 implementation correction: the first effect slice is source-derived inventory plus pointer-free descriptor/lowering infrastructure, not a claim that production observer fields are populated. `GEO_CULLING_RADIUS` and `GEO_BRANCH_AND_LINK` remain explicit fail-closed prerequisites; source-complete host/linked-image tests may prove billboard basis, alpha/translucent mapping, shadow descriptor bounds, stable painter bins, and capacity/mutation behavior, while source capture, final BOB closure, target/Ymir replay, and FPS remain later gates.
- 2026-08-06 Task 22 preflight correction: final BOB closure is not ready for reseal. The generated closure lacks unresolved-edge/capability totals, S64F still has 13 unsupported reasons across 14 records, the only S64P is a provisional 2,339,972-byte world-only root with empty dependencies/zero residency, and no final linker/replay/launch tooling or generic actor cutover exists. Task 22 remains the sole final-link/reseal owner and must bind exact scene/actor/animation/audio hashes into build identity before any target/Ymir/manual/FPS evidence.
- 2026-08-06 Task 23 preflight correction: audible BOB closure is blocked before target capture. The final Task 22 root and authoritative full audio package/seq00 are absent; sourceboot’s event sink/completion path is weak/no-op; the production MC68000 image is heartbeat/proof-only and does not link Task 15/17 engines; and PCM v2 lacks completion/identity status. Task 23 must preserve SH-2 policy versus MC68000 timing/voice ownership, generate a closure manifest from source/package identities, and bind one exact ELF/audio/CUE/profile before any audible/manual claim.
- 2026-08-06 Task 21 Wave 2 rereview correction: `d23ec834` passes the six focused host gates but is SPEC/QUALITY FAIL (C0/I5/M0). The fix round must bind expected opcode as well as ring/lap for pending tickets, prevent unticketed enqueue from bypassing ABA retirement policy, replace the reusable `STATUS_WRITING` bit with an actual bounded tear-safe publication protocol, use saturating wire counters in the live full-ring path, and close the status/opcode legality matrix. FINISHED remains non-applicable to source policy until a tagged identity extension exists.
- 2026-08-06 Task 21 Wave 2 fix-round-1 rereview correction: `59c2a014` closes four prior defects and the direct pending-cursor bypass, but remains SPEC/QUALITY FAIL (C0/I1/M0). Legacy no-ack is only local SH-2 state; the unchanged command wire cannot tell a future MC68000 producer not to emit an acknowledgment. The next fix must either reject all legacy/no-ack enqueue while completion capability is active or add a producer-visible, versioned acknowledgment identity, with late/no-pending ACK mutations.
- 2026-08-06 Task 21 Wave 2 acceptance: `0e5ac8e4` rejects legacy/no-ack commands before any wire access when completion capability is advertised, preserves capability-absent compatibility, and latches malformed/unmatched/duplicate completions until explicit transport recovery. Independent primary and backup rereviews are SPEC/QUALITY PASS, C0/I0/M0; the six serial DLL/MSYS gates pass in 4.2 seconds. This remains a host-only transport contract; no MC68000 producer, source service, package transaction, target/Ymir/manual, audible, or FPS claim is promoted.
...
- [ ] Task 2 — source-complete — commits `f5a0248d`, `8c97fd4e`, `a7ee4299` (reports `264f3b4b`, `5008759e`); independent rereview PASS for the identity/archive slice. Identity 9/9, sourceboot identity 5/5, capture 35/35, archive 12/12, no-build baseline revalidation, and the HWRAM-resident pre-cart identity guard are green. The fresh Pipe-4 image reaches interactive BOB; linked SH-2 symbol/layout, Ymir/manual target evidence beyond this launch, and Task 1 broad native-math gates remain open.
...
- [x] Keep existing Mario functions as feature-off wrappers. `c763590b` binds explicit wrappers around the exact Mario callbacks for `ACTOR_ADMIT/ACTOR_LOWER`; `8192572f` hardens the branch-polarity gate (6/6 host mutations). The world graph remains unchanged until a separately reviewed integration cutover replaces the single Mario pair deliberately.
...
#### Task 16 bounded continuation: snapshot-bank to actor-queue lifecycle handoff

This actor/runtime slice is intentionally host/source-neutral. It accepts an authoritative Task 14 snapshot bank plus caller-supplied Task 16 descriptors, validates identity and ordering, and drives the existing queue/batch lifecycle without deriving geometry, material, or missing production identities. Zero-count production-safe handling and nonzero host fixtures are allowed; no production actor cutover is implied.

- Create `src/port/saturn/gfx/saturn_actor_runtime_handoff.h/.c` and `tools/saturn/actor_runtime_handoff_test.c`; add `verify-actor-runtime-handoff` to `Makefile.saturn.mk` and update `CHANGELOG.md`.
- [x] RED: cover acquire/validate/publish/finalize/consume/retire ordering, exact descriptor identity preservation, stale/wrong bank rejection, pre-acquire immutability, post-acquire quarantine cleanup, terminality before batching, explicit output-consumer acknowledgement before reset/retire, double-operation/generation-wrap rejection, and exact capacity/overflow boundaries.
- [x] Implement the `EMPTY -> ACQUIRED -> QUEUED -> TERMINAL -> BATCHED -> CONSUMED -> RETIRED` lifecycle using existing Task 14 bank, Task 16 queue, and batch types; no parallel ABI or synthetic geometry fields.
- [x] Run focused serialized GREEN for `verify-actor-runtime-handoff` plus `test_actor_runtime_neutrality.py`; the inherited snapshot/queue/batch/render-snapshot binaries compile and pass when run directly under DLL-preflighted MSYS. The native-Python `/d/...exe` runner defect remains an unchecked gate; P2/target/concurrent-claim/Ymir/manual/FPS remain unchecked.
- [x] Commit `963e10d4` with repair `e82759ce`; independent rereview is SPEC/QUALITY PASS, C0/I0/M0. Keep Task 14 registry/typed-field defects, generic meshlet preparation, production drain, and target evidence explicitly open.

#### Task 14 bounded repair: P2 snapshot payload publication

This repair is ABI- and arena-neutral and may proceed before the broader observer-capacity work. `sm64_saturn_actor_instance_bank_capture()` must write captured snapshot bytes through the same cache-through bank alias used by publish/acquire; the source mutation gate must catch any regression to cached payload writes. The slice does not expand the 64-entry observer or claim target visibility.

- Modify `src/port/saturn/gfx/saturn_actor_instance.c` and `tools/saturn/actor_instance_snapshot_test.c`; extend the source-neutrality test and `Makefile.saturn.mk`; update `CHANGELOG.md`.
- [x] RED: mutate the capture path back to cached `bank->snapshots[selected]` writes and prove the static gate rejects it; add an exact payload acquire fixture that distinguishes cache-through-written bytes from stale cached storage.
- [x] Implement the narrow uncached payload-write repair without changing the 188-byte snapshot ABI, two-bank state machine, observer capacity, or arena accounting.
...
- [x] Produce `task-14-lwram-reclaim-preflight-report.md` with a ranked,
  ...
  boundaries, and the smallest implementation sequence. The complete 0x20000
...
`sourceboot_vdp1_cmdts[2][2048]` (0x20000 bytes, 32-byte aligned) from
...
- [x] RED/GREEN bounded owner contract: `39008658` plus
  ...
  and no standalone observer/instance declarations. Repair `82eb9c57`
  scopes the owner mutation harness; the focused gate and independent
  rereview are SPEC/QUALITY PASS, C0/I0/M0. Package-capacity/production
  binding remains a separate unchecked gate.
...
- [x] RED report binds the exact BILLBOARD, ALPHA, TRANSLUCENT, SHADOW, PARTICLE, PROJECTILE, DECAL, reward, and effect arrays to the named digest-pinned oracle, preserving the nine geo-blocked records and their reasons. Capacity, stale identity, inventory/role, and unknown feature-bit mutations are covered.
- [x] Implement existing-billboard-basis selection, alpha/translucent material classes, stable far-to-near bins, bounded shadow descriptors, and source-owned particle/effect lifetime consumption as pointer-free helpers. Preserve VDP2/VDP1 layer ownership; production observer capture and master painter merge remain later gates.
- [x] Test near-plane billboard basis, opaque/translucent interleave, equal-depth stability, master-resolved shadow receiver/depth, effect expiration without renderer-side timer advancement, projectile/reward/effect oracle multiplicity, zero effect budget, stale identity, unknown fields, and overflow fail-closed telemetry. Repair adds zero canonical bank-hash, stale-token, crafted capability/effect/opacity mutations; the focused and serialized waves pass.
- [x] Run GREEN (repair evidence, host/linked-image only; target/Ymir/manual/FPS remain open):

  ```powershell
  powershell -ExecutionPolicy Bypass -File tools\saturn\with-msys-toolchain.ps1 mingw32-make -f Makefile.saturn.mk -j1 verify-actor-effects verify-actor-batches verify-actor-capability-bank verify-actor-family-bank verify-actor-instance-queue
  .\.venv-saturn-tools\Scripts\python.exe tools\saturn\test_bob_actor_capabilities.py --class effect
  ```

- [x] Commit bounded implementation as `1c89a2bb`; repair `f161491a` binds canonical bank identity and shared direct-descriptor validation. Scoped rereview remains required before marking the bounded slice source-complete; final closure/target evidence remains open.
...
Wave 0/1 bounded status:

- [x] RED missing-wrapper and direct warned-call ownership failures recorded.
- [x] Project-owned generic-SMPC cold/recovery wrapper with pre-stage,
  SNDOFF, bounded stopped-state proof, 512-KiB selection, bounded
  READY-heartbeat wait, typed completion and retained raw OREG31 diagnostics,
  manifest-owner callbacks, barrier, and named fail-closed state.
- [x] `verify-audio-sound-cpu-boot`, `verify-soundtest-boot`,
  `verify-pcm-protocol`, and `verify-audio-protocol-v2` pass serially through
  DLL/MSYS preflight.
...
Wave 2 completion/ack ABI (source-complete, host-only):

- [x] RED tests for reserved-header offsets, 32-entry completion-ring/two-lap
  ...
  ACCEPTED versus FINISHED/PREPARED/COMMITTED semantics were run twice; the
  second RED/review pass exposed five remaining contract defects.
- [x] Implement the bounded completion-ring reader/ticket ABI and full 32-bit
  ...
  silent truncation is forbidden. Initial implementation is `d23ec834`, with
  fixes `59c2a014` and `0e5ac8e4`.
- [x] Run the focused serial ABI/transport/MC68000-model host gates and obtain
  independent primary and backup reviews. The six focused gates pass in 4.2
  seconds; both reviews are SPEC/QUALITY PASS, C0/I0/M0. No production
  completion producer, source service, package commit, target/Ymir, or audible
  evidence is claimed in this wave.
- [x] Fix-round-1 RED/GREEN: require expected opcode on ticket retirement and
  make unticketed enqueue use an explicit no-ack path; add a real bounded
  seqlock/version protocol for split 32-bit status; call the saturating wire
  counter helper in the live full-ring path; and reject illegal
  status/opcode combinations; its first rereview found the no-ack mode was not
  producer-visible on the unchanged wire.
- [x] Fix-round-2 RED/GREEN: reject legacy/no-ack enqueue while completion
  capability is advertised, add no-pending/late-ACK fail-closed coverage, rerun
  the six serial gates, and obtain primary and backup rereviews. Both reviews
  pass SPEC/QUALITY PASS, C0/I0/M0; Wave 2 is source-complete while all
  production/audible gates stay open.
