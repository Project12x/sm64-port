...
- [ ] RED: mutate the capture path back to cached `bank->snapshots[selected]` writes and prove the static gate rejects it; add an exact payload acquire fixture that distinguishes cache-through-written bytes from stale cached storage.
- [ ] Implement the narrow uncached payload-write repair without changing the 188-byte snapshot ABI, two-bank state machine, observer capacity, or arena accounting.
- [ ] Run serialized `verify-actor-instance-snapshot verify-render-snapshot-bank verify-actor-instance-queue verify-actor-batches verify-actor-runtime-handoff` plus `test_actor_runtime_neutrality.py`; the inherited native-Python POSIX executable-path defect must remain explicitly reported if it persists.
- [ ] Commit and independently review the repair; keep >64 source-pool slots, skip-zero generation, package reservation, target/P2 race, production drain, Ymir/manual, and FPS gates open.
