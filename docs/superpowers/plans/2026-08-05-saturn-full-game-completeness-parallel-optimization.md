...
- [ ] Task 21 — fix round 1 active/source-incomplete; broader task source-incomplete. Wave 0/1 implementation `7a1bb170` has independent review SPEC/QUALITY FAIL, C0/I2/M0. The bounded wrapper/protocol gates are green, but the active sequence must be reconciled to SNDOFF→stop wait→512-KiB selection, soundtest must select/verify 512-KiB mode, and raw OREG31 telemetry must be retained in project-owned diagnostics and covered by a seam test. Real seq00/AUDIO.DAT/S64P inputs, completion/identity production linkage, sourceboot feature-on, target, Ymir, manual, and audible claims remain blocked.
...
- [x] Project-owned generic-SMPC cold/recovery wrapper with pre-stage,
  512-KiB selection, bounded stop/READY-heartbeat waits, typed completion,
