...
- [ ] Task 20 — fix round 2 active/source-incomplete — bounded implementation `1c89a2bb`, repair `f161491a`; scoped rereview is SPEC/QUALITY FAIL, C0/I1/M1. Bank identity, shared direct-descriptor validation, exact oracle, VDP1 mapping, stable 64-bin painter order, and serialized actor regressions are green. The public lowering API still needs current generation/package validation and the report needs its 68-byte/24-byte ABI wording corrected. Nine geo-blocked records, production observer capture, final closure, target/Ymir replay, and FPS remain open.
- [ ] Task 21 — preflight complete; implementation not started — integrate bounded audio package boot/residency in sourceboot. The preflight and custom-Yaul/MC68000 audit identify the missing service/loader/transport/image links, real seq00/AUDIO.DAT/S64P inputs, completion/identity status, and the safe CDFS/SNDOFF ownership boundary. No sourceboot, target, Ymir, manual, or audible claim is made until the six-wave implementation/review handoff passes.
