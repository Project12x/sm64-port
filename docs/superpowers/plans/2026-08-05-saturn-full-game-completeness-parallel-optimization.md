...
- [ ] Task 21 — active/source-incomplete — preflight and custom-Yaul/MC68000 audit complete; begin bounded source/host audio boot-contract slice. The first wave may implement only the project-owned SMPC/sound-CPU wrapper, cold-boot/recovery ordering, direct-call gate, and fail-closed host contracts. Real seq00/AUDIO.DAT/S64P inputs, completion/identity production linkage, sourceboot feature-on, target, Ymir, manual, and audible claims remain blocked until later waves.
