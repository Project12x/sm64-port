...
- [ ] Task 19 — active/source-incomplete — bounded admission repair `2222c375` is complete and scoped rereview is pending. Exact six-query oracle, resealed stored-mask mutation, unavailable PARENTED/HELD/LOD masks, standardized target, and the full bounded serial wave are green. Runtime pose/queue/lane/parent/held/model/despawn/reward/stale-parent proof remains deferred until Task 14 typed immutable state exists; no production-capability or target/FPS claim is made.
