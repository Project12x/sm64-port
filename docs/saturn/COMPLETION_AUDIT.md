...
| Deterministic WRAM telemetry | Fixed 64-byte `SAT0` block, 56-byte `SATX` block, compile-time size/offset assertions ([offset build record](evidence/hwtest-offset-assert-2026-07-17.md)), strict version/magic and `0x5C`/4 MiB decoder gates in [`telemetry_decode.py`](../../tools/saturn/telemetry_decode.py) | Code contract verified; valid Ymir execution pending |
