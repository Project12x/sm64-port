...
| [Maxime-XL2/SONIC-Z-TREME](https://github.com/Maxime-XL2/SONIC-Z-TREME) | `cff75451c1616aac1236fc2b44223902b55c706b` / GPL-3.0 | `Projects/SONIC Z-TREME/ZTE/ZT_RENDERING.c`: `traverse_octree`, `displaySpriteEnt`, `display_model`, `ztRender`; `ZT_LOADING.c`: `loadPDATA`, `loadLODpdata`, `loadNodes`, `mallocVertices`, `loadBinaryMAP`; `workarea.c`; `ZTE_DEF.H` | **Pattern study so far; direct GPL reuse is authorized.** No renderer source from these files has yet been copied. The inspected implementation supplies concrete candidates for direct-copy or close-port work: bounded fixed work areas, hierarchy AABB/frustum rejection, inherited inside/intersects state, camera-relative near-to-far child traversal, per-leaf full/LOD selection, distant-entity suppression, contiguous level-package loading, and early render-job dispatch. Any reuse must retain the pinned provenance, copyright and GPL notices, change records, and corresponding-source/copyleft compliance. Its warnings about experimental PVS and loose map bounds still require technical validation rather than verbatim adoption. |
| [Lobotomy-Software/SlaveDriver-Engine](https://github.com/Lobotomy-Software/SlaveDriver-Engine) | `a8986591557b6e680550d3c23970284d3b38ff8f` / GPL-3.0 | `WALLS.C`: `SectorDrawRecord`, `clipZSub`, `drawClippedFace`, `drawWall`, `slave_drawWall`, `findDoorways`, `buildTree`, `sortLeafList`, `drawWalls`, `drawWallsFinish`, `frustumClip`, `drawSprites`; `OBJECT.C`: `moveObject`, `processDelayedMoves`; `V_BLANK.C`: `UsrVblankStart`, `UsrVblankEnd`; `DMA.C`, `DMA.H`, `SCL_FUNC.C` | **Pattern study for the world renderer; existing DMA queue is a close-port.** The world-renderer files have not yet been copied, but direct-copy or close-port reuse is authorized where their portal/dependency ordering, sector-local clips and objects, shade-preserving clipping, compact slave results, command linking, or adaptive master/slave partitioning fit the SM64 architecture. The bounded DMA queue is already close-ported under `src/port/saturn/gpl/` as documented in `SLAVEDRIVER_ADAPTATION.md`. All reused code must retain the pinned provenance, copyright and GPL notices, change records, and corresponding-source/copyleft compliance. |
