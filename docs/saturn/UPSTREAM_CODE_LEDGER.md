...
| Upstream | Pin / license | Inspected code | Reuse mode and concrete destination |
| --- | --- | --- | --- |
| `ponut64/SCSP_poneSound` | `31782e4c61337327f23eb9aa45ecd37fe0944ea0` / MIT | `LICENSE`, `README.md`, `documentation.md`, `PROJ/main.c`, `PROJ/linker`, `PROJ/makefile`, `jo_demo/pcmsys.c`, `jo_demo/pcmsys.h` | **Pattern-only** for `saturn_pcm_protocol.h`; **close-port** of only the vector/reset/linker shape in `audio68k/start.S` and `audio68k/linker.ld`. The local image adds a 16 KiB cap, low stack, fixed mailbox, bounded BSS clearing, and an original heartbeat loop. No upstream driver/control/ADX/CDDA code or binary is included. The retained MIT notice and change boundary are in `docs/saturn/audio/PCM68K_PROVENANCE.md`. |
| `yaul-org/libyaul-examples` | `66b648eb059bb8bb7392eac70821605a68205b85` / no repository license file at pin | `scsp-ponesound-pcm8/Makefile`, `ponesound.c`, `ponesound.h`, `scsp-ponesound-pcm8.c` | **Validation/pattern-only.** Confirms separate driver embedding/loading, 512-KiB sound-RAM selection, generic SMPC sound-CPU commands, fixed SCSP-RAM communication, and a VBlank-published PoneSound service request without a Yaul fork. Copy no source or binary assets. The full-RAM clear and VBlank scheduler are explicitly not adopted; see `docs/saturn/audio/PCM68K_PROVENANCE.md`. |
...
## Task 23A Task 6 — dirty-cell diff/publish (2026-08-07)

| Upstream | Pin / license | Inspected code | Reuse mode and concrete destination |
|---|---|---|---|
| [Lobotomy-Software/SlaveDriver-Engine](https://github.com/Lobotomy-Software/SlaveDriver-Engine) | `a8986591557b6e680550d3c23970284d3b38ff8f` / GPL-3.0-or-later | `SCL_FUNC.C:681-743` (`SCL_PriIntProc`, confirmed at the pinned commit to span exactly those 63 lines, and by its own header comment to run once per VBlank-interval interrupt) and `SOUND.C` (`slotDirty[32]`: declared line 24, set on write in `playSoundMegaE()` line 307, checked-then-cleared before reuse in `playSoundE()` lines 335-337); `PRINT.C`/`PRINT.H` (SlaveDriver's own text/HUD-print module, read for the honest-negative-finding check described below) | **Pattern-only.** `src/port/saturn/gfx/saturn_hud_publish.{h,c}` adopts the shape of `SCL_PriIntProc`'s per-VBlank routine -- check a set of dirty markers, flush only the ones actually marked (DMA-copy in the original; a VDP2 PND write here) at one safe call site, then clear each marker as it's consumed -- and `slotDirty[32]`'s corroborating set-on-write/check-before-use/clear-after-consume shape at per-index (per-SCSP-slot) granularity. No SlaveDriver source is copied: this task's actual comparison is a from-scratch (col,row,glyph) record diff against the previous publish's cell list, not a port of either cited flag mechanism. Honest negative finding, checked directly rather than assumed: `PRINT.C` contains no dirty-tracking of any kind and renders text as VDP1 sprites (`sega_spr.h`, `EZ_setLookupTbl`) rather than VDP2 character-pattern cells at all, so it is not even the same rendering mechanism this task's atlas uses, dirty-tracked or not. Sonic Z-Treme (pinned `cff75451c1616aac1236fc2b44223902b55c706b`, already cited above and in Task 4's row for `ztFont2NBG3`) was re-checked for the same reason and this time correctly traced: `draw_stats()` (`ZT_RENDERING.c:146-152`) issues unconditional `slPrintHex`/`slLocate` calls for `LIVES`, `OWNED` ("Rings or weapons", `ZTE_DEF.H:282`), and a `TIMER`-derived clock, with no gate before any of the three (the function's own early-return at line 148 is itself commented out, and the `showDebugStats` gate at line 165 only guards code after these prints). The call chain confirms this runs every frame: `draw_stats()` is called unconditionally for the local player from `ztRender()` (`ZT_RENDERING.c:792-793`, gated only on `currentPlayer->PLAYER_ID == 0`, true for `PLAYER_1`, `main.c:119`), which `main_loop()` calls unconditionally every invocation (`SRC/game.c:772`), inside `ztGameLoop()`'s `while(1)` loop paced by `slSynch()` (`ZT_GAME.c:70-107`). So the plan's original citation is correct: Z-Treme's own live HUD counters do redraw unconditionally every frame, with no dirty tracking of any kind. (An earlier revision of this row instead cited an unrelated commented-out label inside `ztReset()`, `SRC/game.c:30` -- a one-time death/respawn path, not the per-frame render path -- and wrongly concluded no live call site existed; that was a research error that checked the wrong function, corrected here after re-reading the pinned tree directly.) The original negative finding still holds, now on the correct evidence: neither pinned reference implements *per-cell* dirty diffing for VDP2 tile content -- Z-Treme redraws its counters wholesale every frame instead of diffing them, and SlaveDriver's text system is not VDP2-cell-based at all -- so `saturn_hud_publish.c`'s cell-level diff remains original engineering for this project. Full record in `PROVENANCE.md`. |
