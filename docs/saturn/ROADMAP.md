...
rejected**. The guarded `3fef49c` hot1/clip1 target attempt failed before link
because the new standalone terrain-command-template translation unit reaches
`slavedriver_terrain_result.h` without a visible
`CPU_ADDRESS_PARTITION_MASK`. No fresh ELF/CUE or Ymir counter/capture evidence
exists. Commit `23c3cdd` remains the next comparison baseline; the target
compile and unchanged truthful native-math gate must both permit a fresh image
before the serial BOB comparison resumes. See
